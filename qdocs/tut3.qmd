---
title: "Estimating channel geometry from impulse response"
engine: julia
---
...
We see that the estimated values of `h`, `r`, `d1` and `d2` are fairly close to the ground truth values. We can also estimate the uncertainty in the estimates:

```{julia}
# extract standard deviation of distribution over parameters h, r, d1 and d2
σ = std(rand(q, 10000); dims=2)
```
