...
While standard `UnderwaterAcoustics.jl` API functions automatically convert input values to the appropriate `Pos` and units, you may wish to support these features in your own code when extending `UnderwaterAcoustics.jl`. To do so, you may use the unexported function `XYZ(⋯)` to create a position from various different forms. You may also use the unexported `in_units(⋯)` function to convert input values to the appropriate units:
```julia
