...
If no `rmax` is specified with `backscatter=true`, the modeling domain extends to twice the query range, so that features behind the receivers can still generate echoes.
...
- **Modeling domain**: rays exiting the domain ($x < 0$ or $x > \texttt{rmax}$) are assumed to never return, so anything reflective outside the domain is invisible to the model.
