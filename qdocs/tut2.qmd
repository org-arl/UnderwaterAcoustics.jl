---
title: "Geo-acoustic inversion with an acoustic profiler"
engine: julia
---
...
The returned `q` is a 3-dimensional posterior probability distribution over the parameters `ρᵣ`, `cᵣ` and `δᵣ`. We can estimate the mean of the distribution by drawing random variates and taking the sample mean:

```{julia}
mean(rand(q, 10000); dims=2)
```
...
plot(ρ -> pdf(q, [ρ, 1850, 0.001]), 1300, 1700; xlabel="ρ", legend=false)
```
```{julia}
plot(c -> pdf(q, [1500, c, 0.001]), 1650, 1950; xlabel="c", legend=false)
```
```{julia}
plot(δ -> pdf(q, [1500, 1850, δ]), 0.0, 0.003; xlabel="δ", legend=false)
```
